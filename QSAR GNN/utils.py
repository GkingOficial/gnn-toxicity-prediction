...
def load_folds():
    folds = []
    for i in range(5):
        df = pd.read_csv(f"../data/mutagenicity/mutagenicity_fold{i+1}.csv")
        smi = df["Canonical_Smiles"].values
        prop = df["Activity"].values
...
def load_input_HIV():
    ...
    prop_list = []
    # Actives
    f = open('./data/TOX/melanoma_cleaned.txt', 'r')
